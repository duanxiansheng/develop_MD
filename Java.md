...
>
> 3. 如果没报错，使用 java 文件名.不需要加后缀`class` 
...
+ 数据类型
  + 基本数据类型：
    ...
    > 浮点数
    >
    > ​	float			4 	3.4*10^38 > 2*10^38 > 2*8^38  = 2*2^3^38 = 2*2^114
    > ​	double       8
    > 字符
    > ​	char			2
    > 布尔
    ...
    **数据转换类型**c
    ...
    运算时候ASCII码表 字母a= 97 A =65  0 =48 

## 加法运算

字符串数据和其他数据做加法运算，结果是字符串
...
> 逻辑异或
>
> ​	^   1^2为true，1^1为false    // 相同为false 不同为true
